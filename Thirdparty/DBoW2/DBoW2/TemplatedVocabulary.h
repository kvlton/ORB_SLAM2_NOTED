...
template<class TDescriptor, class F> 
void TemplatedVocabulary<TDescriptor,F>::transform(
  const std::vector<TDescriptor>& features,             // 图像的特征描述子
  BowVector &v, FeatureVector &fv, int levelsup) const  // 词袋向量，正向索引map，正向索引的层数（6-4）
{
  v.clear();
  fv.clear();
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
    {
      ...
      if(w > 0)
      { 
        v.addWeight(id, w);            // 累计该叶子节点的权重 v(id) += w
        fv.addFeature(nid, i_feature); // map<正向索引节点，特征ID>
      }
    }
...
template<class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor,F>::transform
(const TDescriptor &feature,           // 特征描述子
 WordId &word_id, WordValue &weight,   // 叶节点ID（叶节点列表中的ID，不是树节点的ID），IDF权重
 NodeId *nid, int levelsup) const      // 正向索引（某一层的父节点ID），正向索引在第（6-4）层
{ 
  // 用来保存每一层的节点ID
  vector<NodeId> nodes;
  ...
  // 保存叶子节点的信息
  word_id = m_nodes[final_id].word_id; // 叶节点ID（叶节点列表中的ID，不是树节点的ID）
...
bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
{
    ...
    // 根据第一行信息，获取字典的 分类数k、树的深度d、L1范数、TF-IDF
    string s;
    getline(f,s);
    stringstream ss;
    ss << s;
