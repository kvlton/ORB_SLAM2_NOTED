...
mfNNratio(nnratio),            // 最优匹配和次优匹配的阈值（相差较大表示匹配较好）
mbCheckOrientation(checkOri)   // 是否根据特征角度的变化来剔除误匹配
{
}
...
{
    const vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches();       // 关键帧的地图点
    vpMapPointMatches = vector<MapPoint*>(F.N,static_cast<MapPoint*>(NULL)); // 当前帧匹配到的地图点，未被匹配到的为NULL
    const DBoW2::FeatureVector &vFeatVecKF = pKF->mFeatVec;                  // 关键帧正向索引map，正向索引节点能搜索到的特征ID
    ...
    for(int i=0; i<LastFrame.N; i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                if(invzc<0)
                    continue;
                float u = CurrentFrame.fx*xc*invzc+CurrentFrame.cx;// 2D像素坐标（当前帧）
...
Frame &CurrentFrame, KeyFrame *pKF,  // 当前帧，关键帧
const set<MapPoint*> &sAlreadyFound, // 已经匹配到的地图点
const float th , const int ORBdist)  // 搜索范围，误匹配阈值
{
    // 匹配到的特征数量
    int nmatches = 0;
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            ...
            if(!pMP->isBad() && !sAlreadyFound.count(pMP))
            {
                ...
                const float yc = x3Dc.at<float>(1);
                const float invzc = 1.0/x3Dc.at<float>(2);
                const float u = CurrentFrame.fx*xc*invzc+CurrentFrame.cx;// 2D像素坐标（当前帧）
